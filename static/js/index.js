  ...
  fetch("./leaderboard_data.json")
    ...
    .then((data) => {
      ...
      data.leaderboardData.forEach((row, index) => {
        ...
        const nameCell =
          row.info.link && row.info.link.trim() !== ""
        ...
        const valOverall = formatOverallValue(
          applyStyle(
            ...
            valScores.overall[index]
          ),
          safeGet(row, "validation.source")
        );
...
function setupEventListeners() {
  ...
  document
    .querySelector(".pro-details-cell")
    .addEventListener("click", function () {
      toggleDetails("pro");
    });
  document
    .querySelector(".val-details-cell")
    .addEventListener("click", function () {
      toggleDetails("val");
    });
  document
    .querySelector(".test-details-cell")
    .addEventListener("click", function () {
      toggleDetails("test");
    });
...
function resetTable() {
  document
    .querySelectorAll(".pro-details, .val-details, .test-details")
...
function getCellValue(row, index) {
  ...
  if (cell.classList.contains("hidden")) {
    if (
      cell.classList.contains("pro-details") ||
      cell.classList.contains("pro-overall")
    ) {
      cell = cells.find(
        (c) =>
          (c.classList.contains("pro-overall") ||
            c.classList.contains("pro-details")) &&
          !c.classList.contains("hidden")
      );
    } else if (
      cell.classList.contains("val-details") ||
      cell.classList.contains("val-overall")
    ) {
      cell = cells.find(
        (c) =>
          (c.classList.contains("val-overall") ||
            c.classList.contains("val-details")) &&
          !c.classList.contains("hidden")
      );
    } else if (
      cell.classList.contains("test-details") ||
      cell.classList.contains("test-overall")
    ) {
      cell = cells.find(
        (c) =>
          (c.classList.contains("test-overall") ||
    ...
    datasets: [
      {
        ...
        backgroundColor: [
          "rgba(196, 123, 160, 0.6)",
          "rgba(245, 123, 113, 0.6)",
          "rgba(255, 208, 80, 0.6)",
          "rgba(110, 194, 134, 0.6)",
          "rgba(255, 153, 78, 0.6)",
          "rgba(42, 149, 235, 0.6)",
          "rgba(183, 156, 220, 0.6)",
          "rgba(143, 169, 209, 0.6)",
          "rgba(72, 199, 176, 0.6)",
          "rgba(117, 209, 215, 0.6)",
        ],
        borderColor: [
          "rgba(196, 123, 160, 1)",
          "rgba(245, 123, 113,0.4)",
          "rgba(255, 208, 80, 1)",
          "rgba(110, 194, 134, 1)",
          "rgba(255, 153, 78, 1)",
          "rgba(42, 149, 235, 1)",
